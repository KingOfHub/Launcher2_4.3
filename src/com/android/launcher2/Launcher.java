...
public final class Launcher extends Activity
    ...
    private static final int MENU_GROUP_WALLPAPER = 1;   //1
    private static final int MENU_GROUP_SORT = MENU_GROUP_WALLPAPER + 1; //2
    private static final int MENU_WALLPAPER_SETTINGS = Menu.FIRST + 1;// +1
    private static final int MENU_MANAGE_APPS = MENU_WALLPAPER_SETTINGS + 1;// +2
    private static final int MENU_SYSTEM_ICON=MENU_MANAGE_APPS+1; // +3
    private static final int MENU_SYSTEM_SETTINGS = MENU_MANAGE_APPS + 1; //+3
    private static final int MENU_SYSTEM_SETTINGS_Q39q = MENU_SYSTEM_ICON + 1; // +4
    ...
    private SharedPreferences myapplistSharedPrefs;
    ...
    private static final String LAUNCHER_WALLPAPER = "persist.env.launcher.wallpaper";
    private static final String LAUNCHER_WALLPAPERDIM = "persist.env.launcher.wpdim";
    private boolean mDisplayWallPaperInAllApp =
        SystemProperties.getBoolean(LAUNCHER_WALLPAPER, false);
    private float mWallPaperDimInAllApp =
        SystemProperties.getInt(LAUNCHER_WALLPAPERDIM, 4)/10f;

    private static final String LAUNCHER_SORT = "persist.env.launcher.sort";

    private static final boolean LUNCHER_SORT_ENABLED =
        SystemProperties.getBoolean(LAUNCHER_SORT, false);
    ...
    protected void onCreate(Bundle savedInstanceState) {
    	...
    	if(Build.PWV_CUSTOM_CUSTOM.equals("SYB")){
    		mDisplayWallPaperInAllApp = true;
    	}
        
        mContext=this;

        super.onCreate(savedInstanceState);
        LauncherApplication app = ((LauncherApplication)getApplication()); // 获取 app 的 application
        ...
        if ((Build.PROJECT.equals("SQ27") || Build.PROJECT.equals("SQ27C"))
                && (Build.PWV_CUSTOM_CUSTOM.equals("KPOS")||Build.PWV_CUSTOM_CUSTOM.equals("SYB"))) {
        ...
        if( Build.PROJECT.equals("ID85")) {
            setContentView(R.layout.launcher_39);
        } else if( Build.PROJECT.equals("SQ27D")) {
            setContentView(R.layout.launcher_sq27d);
        } else{
    ...
    static void changeforlist() {
        mModel.startLoader(true, 2);
    }
    ...
    */
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if(!(Build.PWV_CUSTOM_CUSTOM.equals("WT-M"))) {
            if (Build.PWV_CUSTOM_CUSTOM.equals("KPOS")  || Build.PWV_CUSTOM_CUSTOM.equals("SYB")) {
            ...
            if (!(Build.PWV_CUSTOM_CUSTOM.equals("KQ") || Build.PWV_CUSTOM_CUSTOM.equals("KPOS") ||Build.PWV_CUSTOM_CUSTOM.equals("SYB"))) {
            ...
            if (Build.PROJECT.equals("ID85")) {
                ...
                menu.add(0, MENU_SYSTEM_ICON, 0, R.string.menu_manage_icons)
                        .setIcon(android.R.drawable.ic_menu_manage)
                        .setAlphabeticShortcut('W');
                menu.add(0, MENU_SYSTEM_SETTINGS_Q39q, 0, R.string.menu_settings)
...
else
{
  switch (item.getItemId()) {
    ...
    private void startIconList() {
    	Intent intent = new Intent(); 
        intent.setClass(Launcher.this, LauncherAppsManager.class);
        startActivity(intent);
    }
    private void startWallpaper() {
        ...
        // titleResoureName=com.android.launcher:string/chooser_wallpaper
...
//        }
    ...
    boolean startActivitySafely(View v, Intent intent, Object tag) {
        boolean success = false;
        try {
            if(AllAppsList.mMail189Enabled && intent.getComponent() != null
        ...
        if (allowLongPress && !mDragController.isDragging()) {
            if (itemUnderLongClick == null) {
               ...
               if(Build.PROJECT.equals("ID85")) {
                       ...
                       final String[] items = {getString(R.string.choose_wallpaper),getString(R.string.choose_widget)};
                       ...
                       builder.show();
               } else if ((Build.PROJECT.equals("SQ27") || Build.PROJECT.equals("SQ27C")) &&
                       (Build.PWV_CUSTOM_CUSTOM.equals("KPOS")||Build.PWV_CUSTOM_CUSTOM.equals("SYB"))){
   ...
   void setBehindWindowDim(boolean dimon) {
        if(dimon) {
            ...
            getWindow().setDimAmount(mWallPaperDimInAllApp); //ref: 0.2f
        }
        else {
        ...
        if (animated) {
            ...
            mStateAnimation.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if (!mDisplayWallPaperInAllApp && !animationCancelled) {
                        updateWallpaperVisibility(false);
                    }
   ...
   // void showAllApps(boolean animated) {
    ...
    public void bindAppsAdded(final ArrayList<ApplicationInfo> apps) {
    	 SharedPreferences LauncherApps=getSharedPreferences("LauncherApps",MODE_PRIVATE);
         String applist=LauncherApps.getString("AllowedClassNames", "");
         Editor edit = LauncherApps.edit();
         for(int i=0;i<apps.size();i++) {
        	 ApplicationInfo info = apps.get(i);
            applist+=info.componentName.getPackageName();
            applist+=",";
         }
        edit.putString("AllowedClassNames", applist);
        edit.commit();
        addAppInfo(apps);
        if (waitUntilResume(new Runnable() {
                public void run() {
