...
public class LauncherProvider extends ContentProvider {
    ...
    private static Context sResPackageLayout;
    ...
    private static final String LAUNCHER_SHORTCUT = "persist.env.launcher.shortcut";

    private static final boolean LAUNCHER_SHORTCUT_ENABLED =
        SystemProperties.getBoolean(LAUNCHER_SHORTCUT, false);

    private static final String LAUNCHER_WORKSPACE = "persist.env.launcher.workspace";

    private static final String mlauncherWorkspace =
        SystemProperties.get(LAUNCHER_WORKSPACE);
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
        ...
        if (sp.getBoolean(DB_CREATED_BUT_DEFAULT_WORKSPACE_NOT_LOADED, false)) {
            ...
            if (workspaceResId == 0) {
                if(mlauncherWorkspace != null && "Micromax".equals(mlauncherWorkspace)) {
                    workspaceResId = sp.getInt(DEFAULT_WORKSPACE_RESOURCE_ID,
                        R.xml.micromax_default_workspace);
                } else if (Build.PWV_CUSTOM_CUSTOM.equals("HEADING")) {
					workspaceResId = sp.getInt(
							DEFAULT_WORKSPACE_RESOURCE_ID,
							R.xml.heading_default_workspace);
				} else if (Build.PWV_CUSTOM_CUSTOM.equals("SZT")) {
                    workspaceResId = sp.getInt(
                            DEFAULT_WORKSPACE_RESOURCE_ID,
                            R.xml.default_workspace_szt);
                } else if (!LAUNCHER_SHORTCUT_ENABLED) {
                    try {
                        sResPackageLayout = getContext().createPackageContext(
                                "com.android.Launcher2LayoutRes",
                                Context.CONTEXT_IGNORE_SECURITY);
                    } catch (Exception e) {
                        Log.e("Res_Update", "Create Res Apk Failed");
                    }
					if (Build.PROJECT.equals("ID80")
							&& Build.PWV_CUSTOM_CUSTOM.equals("WJ")) {
						workspaceResId = sp.getInt(
								DEFAULT_WORKSPACE_RESOURCE_ID,
								R.xml.default_workspace_wj);
					} else if (Build.PWV_CUSTOM_CUSTOM.equals("SYB")) {
						workspaceResId = sp.getInt(
								DEFAULT_WORKSPACE_RESOURCE_ID,
								R.xml.default_workspace_wj);
					} else {
						if (sResPackageLayout != null) {
							workspaceResId = sp
									.getInt(DEFAULT_WORKSPACE_RESOURCE_ID,
											sResPackageLayout
													.getResources()
													.getIdentifier(
															"default_workspace",
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private int loadFavorites(SQLiteDatabase db, int workspaceResourceId) {
            ...
            try {
                if (sResPackageLayout != null) {
                    parser = sResPackageLayout.getResources().getXml(
                            workspaceResourceId);
                } else {
                    parser = mContext.getResources()
                            .getXml(workspaceResourceId);
                }
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
            ...
            if (Build.PROJECT.equals("A862") && Build.PWV_CUSTOM_CUSTOM.equals("RCA") && className.equals("com.android.dialer.DialtactsActivity")) {
